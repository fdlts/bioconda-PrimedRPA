...
def pad_fastadict_to_max_length(fastadict, pad_char='-'):
	if not fastadict:
		return fastadict
	max_len = max(len(seq) for seq in fastadict.values())
	return {name: seq.ljust(max_len, pad_char) for name, seq in fastadict.items()}


def aligned_output_name(filename):
	root, ext = os.path.splitext(filename)
	if ext == '':
		ext = '.fasta'
	return f'{root}_Aligned{ext}'
...
def RunningClustalo1(ClustalOPath,
	...
	for FileName in ListOfFileNames:
		OutputName = aligned_output_name(FileName)
...
def CreatingInputHomologyDF(fastadict,FastaIndexList):
	...
	for seqindex in FastaIndexList:
		...
		for faseq in fastadict.values():
			if seqindex < len(faseq):
				TempNucleotides.append(faseq[seqindex])
			else:
				TempNucleotides.append('-')
		...
		AlignedRows.append({'Index_Pos':seqindex,'Nucleotide':MostCommonN,'IdentityScore':NRepresentation})


	return pd.DataFrame(AlignedRows)
								...
								if NoRepeat == True:
									...
									if SDSSFilterPass == True:
										...
										if ProbePass == True:
											...
											if BlastnPass == True:
												OligoRows.append(RowDict)

	# Return Primer/Probe DataFrame
	return pd.DataFrame(OligoRows)
	...
	for Combo in CombosList:
		...
		if DimerisationPass == True:
			...
			if BlastnPass == True:
				...
				PassedSetsRows.append(PassedComboRow)

	return pd.DataFrame(PassedSetsRows)
...
def CheckingAlignedOutputFile(AllParameter):
	...
	#Create Binding DataFrame If It Doesnt Exist
		...
		# Create Alignment DF if it doesnt exist
			...
			# Pad shorter sequences so the alignment summary can still be generated
			fastadict = pad_fastadict_to_max_length(fastadict)
			FastaSeqLength = max(len(fastaseq) for fastaseq in fastadict.values())

			# Generate necessary alignment summary dataframe
			...
			MergedAlignedDF = pd.concat(AlignedDFMultiThreadOupt).reset_index(drop=True)
		...
		PassedOligos = pd.concat(PotentialPrimerProbeOut).reset_index(drop=True)
			...
			for PPL in PossibleProbeLengths:
				...
				for PSST in PrimerStartSiteTranchesTwo:
					if len(FinalOutputDF) < AllParameter.MaxSets: ## Check this threshold setting.
						...
						FinalOutputDF = pd.concat([FinalOutputDF, TempOutputDF], ignore_index=True)
...
if (AllParameter.InputFileType == 'MS' and AllParameter.InputFile != 'NO') :
	...
	AlignedInputFile = aligned_output_name(AllParameter.InputFile)
	if AlignedInputFile not in glob.glob(AlignedInputFile):
		RunningClustalo1(AllParameter.ClustalOPath,[AllParameter.InputFile], overwriteOutput=False)

	AllParameter.InputFile = AlignedInputFile
